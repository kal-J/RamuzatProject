if ($("#tblAsset_income").length && tabClicked === "tab-asset_income") {
        if (typeof (dTable['tblAsset_income']) !== 'undefined') {
            $(".tab-pane").removeClass("active");
            $("#tab-asset_income").addClass("active");
            dTable['tblAsset_income'].ajax.reload(null, true);
        } else {
            dTable['tblAsset_income'] = $('#tblAsset_income').DataTable({
                "dom": '<"html5buttons"B>lTfgitp',
                "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
                order: [[0, 'asc']],
                ajax: {
                    "url": "<?php echo site_url('Asset_income/jsonList') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function (d) {
                        d.status_id = 1;
                ...
                columns: [
                    ...
                    var ret_txt = '<a href="<?php echo site_url("accounts/view"); ?>/'+full.income_account_id+'" title="View account transactions">'+"["+full.account_code+ "]  "+data+'</a>';
                            return ret_txt;
                        }
                    },
                    {data: 'narrative'},
                    {"data": 'id', render: function (data, type, full, meta) {
                            var ret_txt = "";
                                ret_txt += ' &nbsp;<a href="#edit_transaction-modal" data-toggle="modal" class="btn btn-sm btn-default edit_me"><i class="fa fa-pencil-square-o text-warning"></i></a>';
                ...
                buttons: <?php if (in_array('6', $accounts_privilege)) { ?> getBtnConfig('<?php echo $fixed_asset['asset_name']; ?> INCOME DETAILS'), <?php
    } else {
        echo "[],";
    }
    ?>
