if ($("#tblAppreciation").length && tabClicked === "tab-appreciation") {
        if (typeof (dTable['tblAppreciation']) !== 'undefined') {
            $(".tab-pane").removeClass("active");
            $("#tab-appreciation").addClass("active");
            dTable['tblAppreciation'].ajax.reload(null, true);
        } else {
            dTable['tblAppreciation'] = $('#tblAppreciation').DataTable({
                "dom": '<"html5buttons"B>lTfgitp',
                "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
                order: [[0, 'asc']],
                ajax: {
                    "url": "<?php echo site_url('appreciation/jsonList') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function (d) {
                        d.status_id = 1;
                ...
                "initComplete": function( settings, json ){
                    viewModel.appreciation_amount( sumUp( json.data, 'amount' ) );
                 },
                ...
                columns: [
                    ...
                    {"data": 'id', render: function (data, type, full, meta) {
                            var ret_txt = "";
                                ret_txt += '<a href="#add_appreciation-modal" data-toggle="modal" class="btn btn-sm btn-default edit_me"><i class="fa fa-pencil"></i></a>';
    ...
    <?php } ?>
                ...
                buttons: <?php if (in_array('6', $accounts_privilege)) { ?> getBtnConfig('<?php echo $fixed_asset['asset_name']; ?> appreciation details'), <?php
    } else {
        echo "[],";
    }
    ?>
