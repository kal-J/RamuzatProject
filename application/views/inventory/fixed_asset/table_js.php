if ($("#tblInventory").length && tabClicked === "tab-fixed_asset") {
        if (typeof (dTable['tblInventory']) !== 'undefined') {
            $(".tab-pane").removeClass("active");
            $("#tab-fixed_asset").addClass("active");
            dTable['tblInventory'].ajax.reload(null, true);
        } else {
            dTable['tblInventory'] = $('#tblInventory').DataTable({
                "dom": '<"html5buttons"B>lTfgitp',
                deferRender: true,
                ajax: {
                    "url": "<?php echo site_url('inventory/jsonList_fixed_asset') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function (d) {
                ...
                columns: [
                    ...
                    var ret_txt = '<a href="<?php echo site_url("inventory/asset"); ?>/'+full.id+'" title="'+full.description+'">'+data+'</a>';
                            return ret_txt;
                        }
                    },
