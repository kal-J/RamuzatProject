if ($('#tblDividend_payment').length && tabClicked === "tab-dividends") {
            ...
            deferRender: true,
            ajax:{
                    "url": "<?php echo site_url('Dividend_payment/get_member_dividends') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
            ...
            buttons: <?php if(in_array('6', $share_privilege)){ ?> getBtnConfig('Dividend payments <?php if(isset($user)): ?>client#<?php echo $user['id']; ?><?php endif; ?><?php if(isset($get_share_by_id)): ?>share account#<?php echo $get_share_by_id['id']; ?><?php endif; ?>'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
}
}