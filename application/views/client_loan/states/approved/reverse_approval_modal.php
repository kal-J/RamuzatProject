<div class="modal inmodal fade" id="reverse_approval-modal" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
<div class="modal-dialog modal-lg">
<div class="modal-content">

<form method="post" class="formValidate" action="<?php echo base_url();?>loan_state/reverse_approval" id="formReverse_approval">
...
<div class="modal-header">
     ...
     <h4 class="modal-title">
        ...
        if (isset($modalTitle1)) {
            ...
            echo "Reversing Approval Action";
        }
     ?></h4>
     <small class="font-bold">Note: Loan Application will go back to pending approval</small><br>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-lg-8 form-group">
                          <div class="input-group date" data-date-start-date="<?php echo isset($active_month)?date('d-m-Y', strtotime($active_month['month_start'])):date('d-m-Y', strtotime($fiscal_active['start_date'])); ?>" data-date-end-date="<?php echo isset($active_month)?((strtotime(date('d-m-Y'))<(strtotime($active_month['month_end'])))?date('d-m-Y'):date('d-m-Y', strtotime($active_month['month_end']))):((strtotime(date('d-m-Y'))<(strtotime($fiscal_active['end_date'])))?date('d-m-Y'):date('d-m-Y', strtotime($fiscal_active['end_date']))); ?>">
                            <input class="form-control" onkeydown="return false"  autocomplete="false"  required value="<?php echo date('d-m-Y')?>" name="action_date" type="text"><span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                          </div>
                      </div>
                    </div>
