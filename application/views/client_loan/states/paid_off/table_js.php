//Paid off client loan javascript 
         if ($('#tblPaid_off').length && tabClicked === "tab-paid_off") {
         //reinitailizing daterange picker
         daterangepicker_initializer();

                if(typeof(dTable['tblPaid_off'])!=='undefined'){
                    $(".loans").removeClass("active");
                    $("#tab-loans").addClass("active");
                    $("#tab-paid_off").addClass("active");
                    dTable['tblPaid_off'].ajax.reload(null,true);
                }else{
         dTable['tblPaid_off']= $('#tblPaid_off').DataTable({
            "pageLength": 10,
            "processing": true,
            "serverSide": true,
            "language": {
