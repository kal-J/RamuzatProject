       ...
       $id = 0;
        foreach ($all_portfolio_details['data'] as $key=>$value){
            foreach($value as $key1=>$value1){
               ...
               ?>
               <tr id="subtotal1">
              <td><a href='<?php echo site_url('Portfolio_aging/get_category_loan_lists/');echo ++$id ?>' title="View all"><?php echo $key1 ?><?php ?></a></td>
              <?php
       ...
       ?>
        ...
        foreach ($all_portfolio_details_2['data'] as $key=>$value){
            foreach($value as $key1=>$value1){
               ?>
               <tr id="subtotal1">
              <td><a href='<?php echo site_url('Portfolio_aging/get_category_loan_lists/');echo ++$no ?>r' title="View all"><?php echo $key1; ?></a></td>
              <?php
              foreach($value1 as $key2 => $value2) {
                $value3 = is_numeric($value2) ? number_format($value2) : $value2;
                ?>
    ...
    //     $('#btn_print_active_report').css('display', 'none');
        $.ajax({
            ...
            success: function (response) {
                ...
                printJS({printable: 'printable_portfolio_aging_pdf_printout', type: 'html', targetStyles: ['*'], documentTitle: response.sub_title});
            },
