...
if ($("#tblAgingbills").length && tabClicked === "tab-tabular") {
    
    if (typeof (dTable['tblAgingbills']) !== 'undefined') {
        $(".tab-pane").removeClass("active");
        $("#tab-tabular").addClass("active");
        dTable['tblAgingbills'].ajax.reload(null, true);
    } else {
        dTable['tblAgingbills'] = $('#tblAgingbills').DataTable({
            ...
            buttons: <?php if(in_array('6', $report_privilege)){ ?> getBtnConfig('Aging bills Payable'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
    }
    }