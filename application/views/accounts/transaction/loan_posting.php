...
<div class="modal inmodal fade" id="post_loans-modal" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form method="post" class="formValidate" action="<?php echo site_url("journal_transaction/create"); ?>" id="formLoanJournal_transaction">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">
                        <span aria-hidden="true">×</span><span class="sr-only">Close</span>
                    </button>
                    <h4 class="modal-title">
                       General Loan Posting
                    </h4>
                ...
                <div class="form-group row">
                    ...
                    <div class="form-group col-lg-4">
                        <div class="input-group date" data-date-start-date="<?php echo isset($active_month)?date('d-m-Y', strtotime($active_month['month_start'])):date('d-m-Y', strtotime($fiscal_year['start_date2'])); ?>" data-date-end-date="<?php echo isset($active_month)?((date('d-m-Y')<date('d-m-Y', strtotime($active_month['month_end'])))?date('d-m-Y'):date('d-m-Y', strtotime($active_month['month_end']))):((date('d-m-Y')<date('d-m-Y', strtotime($fiscal_year['end_date2']))?date('d-m-Y'):date('d-m-Y', strtotime($fiscal_year['end_date2'])))); ?>" >
                            <input type="text"  onkeydown="return false" class="form-control" name="transaction_date" placeholder="Transaction date" required/>
                    ...
                    <div class="table-responsive">
                        <table class="table table-condensed table-borderless table-striped">
                            ...
                            <!-- ko foreach: general_ledger_accounts-->
                                <tr>
                                    <td>
                                        <select  class="form-control m-b detail_accounts" data-bind='options: $parent.select2accountsById([92,93,27,28,81,82,83,77,105,78,79,80,97]), optionsText: function(account){return account.account_code + " " + account.account_name}, value:general_ledger_account, optionsCaption: "--select--", optionsAfterRender: setOptionValue("id"), attr:{name:"journal_transaction_line["+$index()+"][account_id]"}' data-msg-required="Account must be selected" style="width: 100%" required>
                                    ...
                                    <td data-bind="with: general_ledger_account">
                                        ...
                                        <input type="number" class="form-control" min="0" data-bind='textInput: $parent.debit_amount, attr:{name:"journal_transaction_line["+$index()+"][debit_amount]"},hasfocus: $parent.debit_focus1'/>
                                        <!--/div-->
                                    </td>
                                    <td data-bind="with: general_ledger_account">
                                        <!--div data-bind='visible: (typeof normal_balance_side !== "undefined" && normal_balance_side == 2)'-->
                                            <input type="number" class="form-control" min="0" data-bind='textInput: $parent.credit_amount, attr:{name:"journal_transaction_line["+$index()+"][credit_amount]"},hasfocus: $parent.credit_focus1' />
                                        <!--/div-->
                                    </td>
                                    <td>
