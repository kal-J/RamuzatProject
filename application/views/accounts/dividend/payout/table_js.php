if ($("#tblIncome_line").length){
        dTable['tblIncome_line'] = $('#tblIncome_line').DataTable({
            "dom": '<"html5buttons"B>lTfgitp',
            order: false,
            deferRender: true,
            ajax:{
                    "url": "<?php echo site_url('income_line/jsonlist') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        d.status_id ='1';
                        d.income_id =<?php echo $income['id']; ?>;
                    }
                },
            ...
            columns: [
                {data: 'income_id', render:function(data, type,full,meta){return "<a href='<?php echo site_url("income/view"); ?>/"+data+"'>"+data+"</a>";}},
            ...
            buttons: <?php if(in_array('6', $accounts_privilege)){ ?> getBtnConfig('Income ID#<?php echo $income['id']; ?>'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
}