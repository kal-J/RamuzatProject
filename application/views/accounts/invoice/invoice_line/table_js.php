if ($("#tblInvoice_line").length){
        dTable['tblInvoice_line'] = $('#tblInvoice_line').DataTable({
            "dom": '<"html5buttons"B>lTfgitp',
            order: false,
            deferRender: true,
            ajax:{
                    "url": "<?php echo site_url('invoice_line/jsonlist') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        d.status_id ='1';
            ...
            columns: [
                //{data: 'invoice_id', render:function(data, type,full,meta){return "<a href='<?php echo site_url("invoice/view"); ?>/"+data+"'>"+data+"</a>";}},
            ...
            buttons: <?php if(in_array('6', $accounts_privilege)){ ?> getBtnConfig('Income ID#<?php echo $invoice['id']; ?>'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
}