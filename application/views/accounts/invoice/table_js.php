if ($("#tblInvoice").length && tabClicked === "tab-invoice") {
var dr_options = {
            'Today': [moment(), moment()],
...
daterangepicker_initializer(dr_options,  moment().format("DD-MM-YYYY"),moment().add(180, 'days').format("DD-MM-YYYY"));
    if (typeof (dTable['tblInvoice']) !== 'undefined') {
        $(".tab-pane").removeClass("active");
        $("#tab-invoice").addClass("active");
        dTable['tblInvoice'].ajax.reload(null, true);
    } else {
        dTable['tblInvoice'] = $('#tblInvoice').DataTable({
            "dom": '<"html5buttons"B>lTfgitp',
            order: false,
            deferRender: true,
            ajax:{
                    "url": "<?php echo site_url('invoice/jsonlist') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
            ...
            columns: [
                {data: 'ref_no', render:function(data, type,full,meta){return "<a href='<?php echo site_url("invoice/view"); ?>/"+full.id+"'>"+data+"</a>";}},
                ...
                {data: 'invoice_date', render: function(data, type,full,meta){ if(type=='sort'){return data?(moment(data,'YYYY-MM-DD').format('X')):0;} return data?moment(data,'YYYY-MM-DD').format('D-M-YYYY'):'';}},
                ...
                var ret_txt =due_amount>0?('<a href="#pay_invoice-modal" data-toggle="modal" class="btn btn-sm btn-default pay_invoice" title="Pay the invoice"><i class="fa fa-check text-primary"></i></a>'):'';
            ...
            buttons: <?php if(in_array('6', $accounts_privilege)){ ?> getBtnConfig('Invoice'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
    }
}