if ($("#tblBill_line").length){
        dTable['tblBill_line'] = $('#tblBill_line').DataTable({
            "dom": '<"html5buttons"B>lTfgitp',
            order: false,
            deferRender: true,
            ajax:{
                    "url": "<?php echo site_url('bill_line/jsonlist') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        d.status_id ='1';
            ...
            columns: [
                //{data: 'bill_id', render:function(data, type,full,meta){return "<a href='<?php echo site_url("bill/view"); ?>/"+data+"'>"+data+"</a>";}},
            ...
            buttons: <?php if(in_array('6', $accounts_privilege)){ ?> getBtnConfig('Expense ID#<?php echo $bill['id']; ?>'), <?php } else { echo "[],"; } ?>
            responsive: true
        });
}