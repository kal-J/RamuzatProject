    if ($("#tblSavings_account_pending").length && tabClicked === "tab-savings_account_pending") {
        if (typeof (dTable['tblSavings_account_pending']) !== 'undefined') {
            $(".savings").removeClass("active");
            $("#tab-savings_account_pending").addClass("active");
            $("#tab-savings").addClass("active");
            dTable['tblSavings_account_pending'].ajax.reload(null, true);
        } else {
            dTable['tblSavings_account_pending'] = $('#tblSavings_account_pending').DataTable({
                ...
                columns: [
                    ...
                    {data: 'id', render: function (data, type, full, meta) {
                        var display_btn ="<div>";
                        <?php if(in_array('5', $savings_privilege)){ ?>
                             display_btn += '<a href="#" data-toggle="modal" title="Activate A/C" data-target="#change_states_modal" class="btn btn-sm set_action"><i class="fa fa-check-circle"></i></a>';
                ...
                buttons: <?php if(in_array('6', $savings_privilege)){ ?> getBtnConfig('Savings Accounts pending '), <?php } else { echo "[],"; } ?>
                responsive: true
            });
        }
    }
