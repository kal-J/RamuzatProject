<!-- bootstrap modal -->
<div class="modal inmodal fade" id="bulk_deposit-modal" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" class="formValidate" enctype="multipart/form-data" action="<?php echo base_url();?>transaction/import" id="formBulk_deposit">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">X</span></button>
                <h3 class="modal-title">Bulk Deposit</h3>
            ...
            <div class="modal-body">
                <div class="form-group row">
                    ...
                    <div class="input-group date"   data-date-start-date="<?php echo isset($active_month)?date('d-m-Y', strtotime($active_month['month_start'])):date('d-m-Y', strtotime($fiscal_active['start_date'])); ?>" data-date-end-date="<?php echo isset($active_month)?((strtotime(date('d-m-Y'))<(strtotime($active_month['month_end'])))?date('d-m-Y'):date('d-m-Y', strtotime($active_month['month_end']))):((strtotime(date('d-m-Y'))<(strtotime($fiscal_active['end_date'])))?date('d-m-Y'):date('d-m-Y', strtotime($fiscal_active['end_date']))); ?>">
                        <input  type="text" class="form-control" onkeydown="return false" name="transaction_date" autocomplete="off" data-bind="datepicker: $root.transaction_date_bulk" required/>
                    ...
                    <div class="form-group row">
                    <label class="col-lg-3 col-form-label">Deposit Method <span class="text-danger">*</span></label></label> 
                        <div class="col-lg-3">
