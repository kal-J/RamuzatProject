<!-- bootstrap modal -->
<div class="modal inmodal fade" id="pay_subscription_fees-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" class="formValidate" action="<?php echo site_url(); ?>client_subscription/create" name="formDefaulters_subscription_fees" id="formDefaulters_subscription_fees">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                    <h3 class="modal-title">Subscription Payment</h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-lg-10">
                            <input name="member_name" id="member_name" class="form-control m-b" disabled>
                        </div>
                    </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-lg-4">
                            <input class="form-control" name="plan_name" id="plan_name" disabled type="text">
                        </div>
                        <label class="col-lg-2 col-form-label">Subscription Date</label>

                        <div class="col-lg-4">
                            <input name="subscription_date" id="subscription_date" class="form-control m-b" disabled>
                        </div>
                    </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-lg-4">
                            <div class="input-group date" data-date-start-date="<?php echo isset($active_month) ? date('d-m-Y', strtotime($active_month['month_start'])) : date('d-m-Y', strtotime($fiscal_active['start_date'])); ?>" data-date-end-date="<?php echo isset($active_month) ? ((strtotime(date('d-m-Y')) < (strtotime($active_month['month_end']))) ? date('d-m-Y') : date('d-m-Y', strtotime($active_month['month_end']))) : ((strtotime(date('d-m-Y')) < (strtotime($fiscal_active['end_date']))) ? date('d-m-Y') : date('d-m-Y', strtotime($fiscal_active['end_date']))); ?>">
                                <input type="text" onkeydown="return false" name="transaction_date" id="transaction_date" class="form-control m-b" autocomplete="off" required="required"><span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                            </div>
                        </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-lg-4">
                            <select data-bind='options: payment_modes, optionsText: "payment_mode", optionsCaption: "-- select --" ,
                        optionsAfterRender: setOptionValue("id"),value: payment_mode, attr:{name:"payment_id"}' class="form-control" style="width: 170px;"> </select>
                        </div>
                        ...
                        <div class="col-lg-4" data-bind="visible: id !=5">
                            <select class="form-control" data-bind='options: $root.transaction_channels, 
                        optionsText:function(data_item){return data_item.channel_name}, optionsCaption: "--select--", 
                        optionsAfterRender: setOptionValue("id"), attr:{name:"transaction_channel_id"}, value:$root.trans_channel' data-msg-required="Transaction channel must be selected" style="width: 100%">
                            </select>
                        </div>
                        <!--/ko-->
                    </div>
