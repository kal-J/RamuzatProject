...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $amount=$loan_data[14];
        $loan_product_id =$loan_data[6];
        $interest_type =$loan_data[16];
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                $single_row = [
                    ...
                    "loan_purpose" => ($loan_data[6] <= 4)  ? "Business" : (
                        $loan_data[6] == 5 ? "SCHOOL FEES" : (
                            $loan_data[6] >=6 ? "AGRICULTURE" : "N/L"
                        )
                    ),
                    "preferred_payment_id" => 1,
                    'date_created' => $date_created,
                    "created_by" => $loan_data[12]
                ];
                ...
                if ($inserted_loan_id) {
                    ...
                    $r=$interest_rate_per_annum=$interest_rate_per_installment=(intval($loan_data[4])*12)/100; 
