...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $topUp = 0;
        $date_created = time();
        $amount = intval($loan_data[5]);
        $member_id = $loan_data[1];
        $loan_product_id =1;
        $repayment_frequency =$loan_data[6];
        $repayment_made_every =3;

        // repayment date from m/d/y to yyyy-mm-dd.
        $repaymentDateCsv = $loan_data[2];
        ...
        $action_date   = date('Y-m-d',strtotime($repaymentDate));
       
        $repaymentDate2 =$action_date;
        // on reducing 
        $interest_type =1;
        $interest_rate = intval($loan_data[3]);
        $installments=1;

        try {
            if ($loan_data[5] != "" && $loan_data[5] != 'NULL') {
                $single_row = [
                    "member_id" => $member_id,
                    "loan_no" => $loan_data[15],
                    "credit_officer_id" => 2, 
