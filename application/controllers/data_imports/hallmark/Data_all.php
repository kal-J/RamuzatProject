 ...
 *
 * @author REAGAN enhanced and imported by Ambrose Ogwang 2022/05/08.
 */
class Data_all extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->member_no="HM00000";
       
    }

    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."hallmark".DIRECTORY_SEPARATOR;
        $file_name = "hallmark_account_balances.csv";
        ...
        if ($customer_data[1] != "" && $customer_data[1] != NULL) {
            ...
            $single_row = [
               ...
               // "email" => isset($customer_data[5])?strtolower($customer_data[5]):'',
                "marital_status_id" => 1, // Always leave as 1 if not going to override
    ...
    public function create_savings_accounts($member_id,$customer_data) {
        ...
        $data = [
            ...
            "account_no" =>strtoupper($this->account_no),
            "date_created" => time(),
            "created_by" => 1,
            "modified_by" => 1
        ];
    ...
    private function create_share_accounts($member_id,$customer_data) {
        ...
        $data = [
            ...
            "share_account_no" => strtoupper($this->share_account_no),
            "date_created" => time(),
            "created_by" => 1,
            "modified_by" => 1
    ...
    private function insert_saving_transaction_data($member_id,$customer_data,$savings_account) {  
        ...
        $credit = $customer_data[6]>0?floatval($customer_data[6]):0;
       
            $trans_row = [
                "transaction_no" =>  date('yws').mt_rand(1000000,9999999),
                "member_id" => $member_id,
                "account_no_id" => $customer_data[2],
                "debit" => NULL,
                "credit" => $credit,
                "transaction_type_id" => 2,
                "payment_id" =>2,
                "transaction_date" => $transaction_date,
                "narrative" =>"Imported savings balance ". "transaction for [ ".$customer_data[1]." ]",
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1,
                'deposit_product_id' => 1
            ...
            $transaction_data=$this->Data_import_model->add_transaction($trans_row);
             ...
             $single_row = [
                ...
                "description" => "Imported savings balance ". "transaction for [ ".$customer_data[1]." ]",
                "transaction_date" =>  $transaction_date,
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1,
    ...
    public function insert_share_transaction_data($member_id,$share_accounts,$customer_data)
    {
      // 5 shares hold transaction for account ids [3,71,75,78,58]
        $transaction_date = "2022-03-31";
        $credit = $customer_data[5] > 0 ? floatval($customer_data[5]):0;
       
        $trans_row = [
            "transaction_no" =>  date('yws') . mt_rand(1000000, 9999999),
            "share_account_id" => $customer_data[2],
            "share_issuance_id" => 1,
            "debit" => NULL,
            "credit" =>  $credit ,
        ...
        if (!empty($transaction_data)) {
            ...
            $single_row = [
                ...
                "ref_id" => $transaction_data['transaction_id'],
                "ref_no" => $transaction_data['transaction_no'],
                "description" => $_SESSION['org_name']." Share balance as [ ".$transaction_date." ]",
                "transaction_date" =>  $transaction_date,
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1,
