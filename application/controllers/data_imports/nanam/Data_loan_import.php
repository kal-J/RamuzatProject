...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
       ...
       // $periods = ["DAYS" => 1, "WEEKS" => 2, "MONTHS" => 3];
        ...
        $repaymentDateCsv = $loan_data[12];
        $repaymentDateCsvExploded = explode("/",$repaymentDateCsv);
        $repaymentDate =$repaymentDateCsvExploded[2]."-"."0".$repaymentDateCsvExploded[0]."-".$repaymentDateCsvExploded[1];
      
        
        // on reducing 
        $interest_type =2;
        $interest_rate = intval($loan_data[9]);
        $installments=1;
