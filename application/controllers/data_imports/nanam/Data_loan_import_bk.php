 ...
 *
 * @author Eric , refactored and imported by Ambrose 
 */
class Data_loan_import extends CI_Controller {
        ...
        if ($handle) {
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    $field_names = $data1;
                    if ($field_names[0] != "Account_No_ID") {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $top_up ='N';
        $loan_product_id =1;
        $repayment_frequency =$loan_data[11];
        $repayment_made_every =3;
        //$interest_rate = ($loan_data[9] == 16)?$loan_data[9]: ($loan_data[9] *12);
        $interest_rate = intval($loan_data[9])*12;
        ...
        if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
            $single_row = [
                ...
                "comment" => 'Loan data imported from the Excel',
                ...
                "approved_by" => 1,
                "approval_note" => 'Loan data imported from excel',
                "loan_purpose" => 'N/L',
                "preferred_payment_id" =>'CASH',
                'date_created' => time(),
                "created_by" => 1
            ];
