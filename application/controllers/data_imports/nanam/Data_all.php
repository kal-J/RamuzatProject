...
class Data_all extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."nanam".DIRECTORY_SEPARATOR;
        $file_name = "nanam_data.csv";
    ...
    private function insert_user_data($customer_data) {
        // Transposing the CVS m/d/y format to d/m/y
        if($customer_data[7] !="" && $customer_data[7]!=NULL){
         $monthDayYearFromCsvExploded = explode("/",$customer_data[7]);
        }
         $day   = isset($monthDayYearFromCsvExploded[1]) ?  $monthDayYearFromCsvExploded[1]:'';
         $month = isset($monthDayYearFromCsvExploded[0] )?  $monthDayYearFromCsvExploded[0]:'';
         $year  = isset($monthDayYearFromCsvExploded[2]) ?  $monthDayYearFromCsvExploded[2]:'';
         $dateOfBirth= $day && $month && $year !=NULL ? $dateOfBirth = $year."-".$month."-".$day:'';
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            $single_row = [
                ...
                "gender" => isset($customer_data[2]) ? $customer_data[2]:'',
            ...
            $member_data = [
                ...
                "occupation" =>isset($customer_data[6]) ? ucwords($customer_data[6]):'N/L',
                "registered_by" => 1,
    ...
    private function create_share_accounts($member_id,$customer_data) {
        ...
        if($customer_data[0]==3 ||$customer_data[0]==71|| $customer_data[0]==58 ||$customer_data[0]==78 ||$customer_data[0]==75)
        {
        $data = [
            "member_id" => $member_id,
            "status_id" =>1,
