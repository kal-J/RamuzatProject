...
class Data_loan_import extends CI_Controller {
    ...
    private function run_updates($file_path) {
        ...
        if ($handle) {
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    ...
                    $client_loans[$data1[1]][$data1[29]]['current']=$data1[0];
                    $parent_loan=isset($client_loans[$data1[1]][$data1[29]]['previous'])?$client_loans[$data1[1]][$data1[29]]['previous']:'';
                    $total_loans = $total_loans + $this->insert_loan_data($data1,$parent_loan);
                    $client_loans[$data1[1]][$data1[29]]['previous']=$data1[0];
                }
                $count++;
            }
            fclose($handle);
        ...
        // print_r($client_loans);
        return $feedback;
    }

    private function insert_loan_data($loan_data,$parent_loan) {
