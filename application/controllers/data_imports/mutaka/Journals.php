...
class Journals extends CI_Controller {
            ...
            while (($data = fgetcsv($handle, 10240, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    ...
                    if ($field_names[0] != "DATE") {
                        $feedback['message'] = "Please ensure that the first cell contains the key DATE  ID";
                        fclose($handle);
                        return $feedback;
                    }
    ...
    private function insert_transaction_data($transaction) {  
        $transaction_date = $this->extract_date_time_dot($transaction[0],"Y-m-d");

            $debit_amount1=$transaction[5];
                $credit_amount1=NULL;
                $debit_amount2=NULL;
                $credit_amount2=$transaction[5];

                $account_ONE=$transaction[1];
                $account_TWO=27;

             $single_row = [
                "journal_type_id" =>2,
                "ref_id" => NULL,
                "ref_no" => $transaction[2],
                "description" =>$transaction[3]." - [ ".$transaction[4]." ] " ,
            ...
            $insert_id=$this->Data_import_model->add_journal_tr($single_row);
             if(!empty($insert_id)){
                ...
                $data[0] = [
                    ...
                    'reference_id' => NULL,
                    'reference_no' => $transaction[2],
                    'transaction_date' => $transaction_date,
                    'narrative' =>$transaction[3]." - [ ".$transaction[4]." ]  made on " . $transaction_date,
    ...
    public function do_insert_accounts($account_data) {
        ...
        //echo $transaction[0] $this->helpers->extract_date_time($account_data[6]); die();
        $data = [
