...
class Member_import extends CI_Controller {
    ...
    private function insert_user_data($customer_data) {
        $date_registered = $this->extract_date_time_dot($customer_data[7],"Y-m-d");
        $date_of_birth = $this->extract_date_time_dot($customer_data[3],"Y-m-d");
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            if ($customer_data[5]) {
                $this->do_insert_contacts($user_id,$this->clean($customer_data[5]));
            }
            if ($customer_data[6]) {
            ...
            $this->create_savings_accounts($member_id,$client_no);
            return 1;
        }
        return 0;
    }
public function clean($string) {
   ...
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
    ...
    public function create_savings_accounts($member_id,$ccount_no) {
        $data = [
            "member_id" => $member_id,
            "status_id" =>7,
            "client_type" =>1,
            "deposit_Product_id" =>1,
            "account_no" => $ccount_no,
            "date_created" => time(),
            "created_by" => 1,
            "modified_by" => 1,
        ];
