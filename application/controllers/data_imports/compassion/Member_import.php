...
class Member_import extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->account_no="SV001";
        $this->share_account_no="SH001";
        $this->member_no= "CSS001";
    ...
    private function run_updates($file_path) {
        ...
        if ($handle) {
            ...
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if(strlen($data1[0])==1){
                $this->member_no = "CSS00".$data1[0];
                $this->account_no = "SV00".$data1[0];
                $this->share_account_no = "SH00".$data1[0];
                 }
                elseif(strlen($data1[0])==2){
                        $this->member_no = "CSS0".$data1[0];  
                        $this->account_no = "SV0".$data1[0];
                        $this->share_account_no = "SH0".$data1[0];
                        }
                     else{
                      $this->member_no = "CSS".$data1[0];
                      $this->account_no = "SV".$data1[0];
                      $this->share_account_no = "SH".$data1[0];
                     }
                     ++$this->account_no;
                     ++$this->share_account_no;
                     ++$this->member_no;
                  }
                $count++;
            }
