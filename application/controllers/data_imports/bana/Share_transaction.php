...
class Share_transaction extends CI_Controller {
    ...
    public function index() {
        ...
        $this->db->trans_start();
        $folder = "data_extract".DIRECTORY_SEPARATOR."kca".DIRECTORY_SEPARATOR;
        ...
        if ($handle) {
            while (($data = fgetcsv($handle, 10240, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    ...
                    if ($field_names[6] != "ID") {
                        $feedback['message'] = "Please ensure that the first cell (ID) contains the key Account No";
                        fclose($handle);
                        return $feedback;
                    }
    ...
    private function insert_transaction_data($transaction) {  
        $transaction_date = $this->helpers->extract_date_time(date('d/m/Y'),"Y-m-d");
         //echo $transaction_date; die();

            $trans_row = [
                "transaction_no" =>  date('yws').mt_rand(1000000,9999999),
                "share_account_id" => $transaction[6],
                "share_issuance_id"=>1,
                "debit" => NULL,
                "credit" => $transaction[4] * 10000,
                "transaction_type_id" => 9,
                "payment_id" =>2,
                "transaction_date" => $transaction_date,
                "narrative" => "Bought " . $transaction[4]." for [ ".$transaction[3]." ]",
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1
            ];
            $transaction_data=$this->Data_import_model->add_transaction_shares($trans_row);
             ...
             $single_row = [
                ...
                "description" =>"Bought " . $transaction[4]." for [ ".$transaction[3]." ]",
                "transaction_date" =>  $transaction_date,
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1,
            ...
            //print_r($insert_id); die;
             ...
             if(!empty($insert_id)){
                ...
                $data[0] = [
                    ...
                    'credit_amount' =>$transaction[4] * 10000,
                    'narrative' => $transaction[4]."[ ".$transaction[3]." ] made on " . $transaction_date,
                    'account_id' =>32,
                    'status_id' => 1
                ];
                $data[1] = [
                    ...
                    'reference_id' => $transaction_data['transaction_id'],
                    'reference_no' => $transaction_data['transaction_no'],
                    'debit_amount' => $transaction[4] * 10000,
                    'narrative' => "Bought " . $transaction[4]." for [ ".$transaction[3]." ] on " . $transaction_date,
                    'account_id' => 27,
                    'status_id' => 1
                ];
