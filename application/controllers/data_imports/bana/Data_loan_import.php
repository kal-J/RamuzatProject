...
class Data_loan_import extends CI_Controller {
        ...
        if ($handle) {
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    $field_names = $data1;
                    if ($field_names[0] != "LOAN_ID/NO") {
                        $feedback['message'] = "Please ensure that the first cell (A1) contains the key LOAN_ID/NO";
                        fclose($handle);
                        return $feedback;
                    }
                } else {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $date_created = time();
        $action_date = date('Y-m-d',strtotime($loan_data[7]));

        //$action_date = $this->helpers->extract_date_time($loan_data[9], "Y-m-d");

        // Compute Next Pay Date
        if($loan_data[10] == 2) {
           $next_pay_date = date("Y-m-d", strtotime($action_date . " + 1 week")); 
        } else if($loan_data[10] == 3) {
            $next_pay_date = date("Y-m-d", strtotime($action_date . " + 1 months")); 
        } else if($loan_data[10] == 1) {
            $next_pay_date = date("Y-m-d", strtotime($action_date . " + 1 day")); 
        }
        ...
        try {
            if ($loan_data[1] != "" && $loan_data[1] != 'NULL') {
                $single_row = [
                    ...
                    "disbursement_note" => "Loan Imported from old system/ Excel",
                    ...
                    "loan_purpose" => $loan_data[6],
                    "preferred_payment_id" => 1,
                    'date_created' => $date_created,
                    "created_by" => 2,
                    "penalty_calculation_method_id" => 1,
                    "penalty_tolerance_period" => 0,
                    "penalty_rate_charged_per" => 3,
                    "penalty_rate" => 0
                ];
                ...
                if ($inserted_loan_id) {
                    ...
                    $r=$interest_rate_per_annum=$interest_rate_per_installment=(intval($loan_data[16]))/100; 
