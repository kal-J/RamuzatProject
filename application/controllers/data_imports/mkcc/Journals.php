...
class Journals extends CI_Controller {
    ...
    private function insert_transaction_data($transaction) {  
        $transaction_date = $this->helpers->extract_date_time($transaction[0],"Y-m-d");

                $debit_amount1=$transaction[2];
                $credit_amount1=NULL;
                $debit_amount2=NULL;
                $credit_amount2=$transaction[2];

                $account_ONE=$transaction[4];
                $account_TWO=40;
             ...
             $single_row = [
                ...
                "description" =>$transaction[1]." - [ ".$transaction[3]." ] " ,
            ...
            $insert_id=$this->Data_import_model->add_journal_tr($single_row);
             if(!empty($insert_id)){
                ...
                $data[0] = [
                    ...
                    'transaction_date' => $transaction_date,
                    'narrative' => $transaction[1]." - [ ".$transaction[3]." ] made on " . $transaction_date,
