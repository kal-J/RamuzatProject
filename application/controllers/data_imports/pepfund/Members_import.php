 ...
 * @author Reagan
 */
class Members_import extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."pepfund".DIRECTORY_SEPARATOR;
    ...
    private function insert_user_data($customer_data) {
        $date_registered = $this->extract_date_time("01-01-2021","Y-m-d");
        $date_of_birth = $this->extract_date_time(isset($customer_data[6])?$customer_data[6]:"01-01-2021","Y-m-d");
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            $single_row = [
                ...
                "gender" =>  isset($customer_data[4])?$customer_data[4]:'M',
                "marital_status_id" => 1,
                "children_no" => 0,
            ...
            if ($customer_data[3]) {
                $this->do_insert_contacts($user_id,$this->clean($customer_data[3]));
            }
            // if ($customer_data[6]) {
            ...
            if ($customer_data[7]) {
                $this->insert_nextofkin($user_id, $customer_data[7]);
            }
            if ($customer_data[8]) {
                $this->insert_nextofkin($user_id, $customer_data[8]);
            }
            if ($customer_data[9]) {
                $this->insert_nextofkin($user_id, $customer_data[9]);
            }
   ...
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
    ...
    private function insert_nextofkin($user_id, $names) {
        $nok_names = explode(" ", $names);
