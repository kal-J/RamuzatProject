...
class Data_all extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."nkd".DIRECTORY_SEPARATOR;
        $file_name = "nkd_data.csv";
    ...
    private function insert_user_data($customer_data){
        ...
        First scenario is to update the existing accounts for members,shares 
        and saving with the new number format
        */

         if($customer_data[0] !="" && $customer_data[0] !=NULL){
            $existingMemberClientNo[] = $customer_data[0];
      
        foreach($existingMemberClientNo as $sysClientNo){
             $oldClientNo = $sysClientNo;
             $where ="fms_member.client_no='".$oldClientNo."'";
             $clientID= $this->member_model->confirmCsvClientNo($where);

             foreach($clientID as $clientNo){
                $currentClientNo = $clientNo;
                $member_id =  $currentClientNo['id'];
                $data['old_client_number'] = $currentClientNo['client_no'];
                $data['new_client_number'] = strtoupper($customer_data[1]);
                $data['member_id'] = $member_id;
                $this->updateToNewNumberFormat($data);
        ...
        if ($customer_data[1] != "" && $customer_data[1] != NULL) {
            ...
            $single_row = [
                ...
                "gender" => (int)$customer_data[3]==1 ? 1:($customer_data[3]==0?0:3),
            ...
            $member_data = [
                "user_id" => $user_id,
                "client_no" => isset($customer_data[1]) ? strtoupper($customer_data[1]):'',
                "branch_id" => 1, 
