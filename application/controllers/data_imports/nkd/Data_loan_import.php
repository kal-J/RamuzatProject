...
class Data_loan_import extends CI_Controller {
        ...
        if ($handle) {
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    $field_names = $data1;
                    if ($field_names[0] != $field_names[0]) {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $dates = explode(".", $loan_data[4]);
        $disbursement_date = '20' . trim($dates[2]) . '-' . trim($dates[1]) . '-' . trim($dates[0]);
        $disbursement_date = date("Y-m-d", strtotime($disbursement_date));

        $action_date = "2022-03-31";
        // Compute Next Pay Date

        $next_pay_date = date("Y-m-d", strtotime($action_date . " + 1 months"));

        $member_id = $this->Data_import_model->get_member_id_from_member_no($loan_data[2])['id'];
        $amount=$loan_data[9];
        $loan_product_id =$loan_data[14];
        $interest_type =$loan_data[14];
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                $single_row = [
                    ...
                    "suggested_disbursement_date" => $disbursement_date,
                    "interest_rate" => $loan_data[13]*12,
                    "offset_period" => 0,
                    "offset_made_every" => 1,
                ...
                if ($inserted_loan_id) {
                    ...
                    $r=$interest_rate_per_annum=$interest_rate_per_installment=(intval($loan_data[13])*12)/100; 
                    ...
                    for ($y=1; $y <= $installments; $y++) { 
                        ...
                        $interest_data[$index_key-1]=[
                            ...
                            $debit_or_credit3=> $loan_data[10],
                            'narrative'=> strtoupper("Interest on Loan Disbursed on ".$disbursement_date),
                            'account_id'=> 6,
                            'status_id'=> 1
                        ];
