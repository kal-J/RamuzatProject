...
class Data_loan_import extends CI_Controller {
    ...
    public function index() {
        $this->db->trans_start();
        $folder = "data_extract".DIRECTORY_SEPARATOR."katalecash".DIRECTORY_SEPARATOR;
        $file_name = "LOANS.csv";
    ...
    private function insert_user_data($customer_data) {
        ...
        if ($customer_data[0] != "" && $customer_data[2] != NULL) {
            ...
            if ($customer_data[1]) {
                $this->do_insert_contacts($user_id,"0".$customer_data[1]);
            }
           
            $this->insert_loan_data($member_id,$customer_data);
    ...
    private function insert_loan_data($loan_data) {
        ...
        $action_date = $this->helpers->extract_date_time($loan_data[3], "Y-m-d");
        ...
        $amount=$loan_data[4];
        $loan_product_id =$loan_data[8];
        $interest_type =$loan_data[6];
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                $single_row = [
                    ...
                    "loan_purpose" => ($loan_data[8] == 1)  ? "MICRO ENTEPRIZE" : (
                        $loan_data[8] == 2 ? "AGRICULTURE" : (
                            $loan_data[8] ==3 ? "PASTOR" : "N/L"
                        )
                    ),
                ...
                if ($inserted_loan_id) {
                    ...
                    $r=$interest_rate_per_annum=$interest_rate_per_installment=(intval($loan_data[5])*12)/100; 
