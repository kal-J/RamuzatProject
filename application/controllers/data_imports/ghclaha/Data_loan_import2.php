 ...
 * @author Eric
 */
class Data_loan_import2 extends CI_Controller {
    ...
    public function index() {
        ...
        $file_name = "ghclaha_loan_2020.csv";
    ...
    private function insert_loan_data($loan_data) {
       ...
       // $repaymentDate =$repaymentDateCsvExploded[2]."-".$repaymentDateCsvExploded[1]."-".$repaymentDateCsvExploded[0];
        $action_date   = date('Y-m-d',strtotime($loan_data[9]));
        $repaymentDate =$action_date;
        // on reducing 
        $interest_type =2;
        $interest_rate = intval($loan_data[4]);
        $installments=1;

        try {
            if ($loan_data[7] != "" && $loan_data[7] != 'NULL') {
                $single_row = [
                    "member_id" =>  $member_id,
                ...
                if ($inserted_loan_id) {
                    ...
                    //Schedule generation
                    $repayment_made_every=3;
                    $next_pay_date = date('Y-m-d',strtotime('+'.$repayment_frequency.' month', strtotime($action_date)));
                    /*
                    ...
                    */
                    ...
                    for ($y=1; $y <= $installments; $y++) { 
                       ...
                       /* if ($interest_type ==2) {
                          ...
                          $principal_amount = $loan_data[7];
                          $interest_amount = $loan_data[8];
