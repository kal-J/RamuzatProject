...
class Member_import extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."buladde".DIRECTORY_SEPARATOR;
    ...
    private function insert_user_data($customer_data) {
        $date_of_birth = $this->helpers->extract_date_time((isset($customer_data[3]) && $customer_data[3] != NULL)?$customer_data[3]:date('d-m-Y'),"Y-m-d");
        $date_registered = $this->helpers->extract_date_time((isset($customer_data[9]) && $customer_data[9] != NULL)?$customer_data[9]:date('d-m-Y'),"Y-m-d");
        $gender = ["M" => 1, "F" => 0];
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            $single_row = [
                ...
                "gender" => $gender[($customer_data[2])?trim($customer_data[2]," "):"F"],
                "marital_status_id" => $marital_status[($customer_data[4])?ucfirst(strtolower(trim($customer_data[4]," "))):"Na"],
                "date_of_birth" => $date_of_birth,
                "children_no" => $customer_data[10],
                "dependants_no" => $customer_data[11],
                "status" => 1,
                "date_created" => time(),
                "created_by" => 1,
                "modified_by" => 1
    ...
    private function insert_nextofkin($user_id, $customer_data) {
        ...
        $nok_names = explode(" ", trim($customer_data[12]));
        ...
        if (isset($nok_names[0])) {
            $data = [
                ...
                "gender" => $this->gender((isset($customer_data[22]) && $customer_data[22] !='') ? ucfirst(strtolower(trim($customer_data[22]," "))) : 'Colleague'),
                "relationship" => $this->relationships[ (isset($customer_data[22]) && $customer_data[22] !='') ? ucfirst(strtolower(trim($customer_data[22]," "))) : 'Colleague'],
                "share_portion" => 100,
                "address" => (isset($customer_data[24]))?$customer_data[24]:'NIL',
                "telphone" => (isset($customer_data[23]) && strlen($customer_data[23]) == 9)?'0'.$customer_data[23]:'',
                "active" => 1,
                "date_created" => time(),
                "created_by" => 1,
                "modified_by" => 1,
