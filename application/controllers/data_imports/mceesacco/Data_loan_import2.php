...
class Data_loan_import2 extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $date_created = time();
        $action_date = $this->helpers->extract_date_time($loan_data[11],"Y-m-d");
        
       
        //$action_date = date('Y-m-d', strtotime('2022-01-01' . ' -' . $loan_data[10] . 'months'));
        ...
        $loan_interest_rate = $loan_data[8];
...
private function insert_loan_payment_transaction($loan_data, $loan_id) {
    ...
    $paid_principal = $loan_data[6]-$loan_data[7];
