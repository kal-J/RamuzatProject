...
class Member_import extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."mops".DIRECTORY_SEPARATOR;
    ...
    private function insert_user_data($customer_data) {
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            if ($customer_data[4] != 'NULL' && $customer_data[4] !='0') {
                $this->do_insert_contacts($user_id,$customer_data[4]);
            }
            if ($customer_data[7] != 'NULL' && $customer_data[7] !='0') {
                $this->insert_nextofkin($user_id,$customer_data);
            }
            $this->create_savings_accounts($member_id);

            if (trim($customer_data[10]) >0) {
                $data = array(
                        "member_id" =>$customer_data[0],
                        "member_fee_id" =>1,
                        "amount" =>trim($customer_data[10]),
                        "narrative" =>"Fees Payment imported on [08-FEB-2021]",
                        "member_name" => $customer_data[2],
                        "account_id" => 45
                    );
                $transaction_date = date('Y-m-d',strtotime('2019-04-01'));
                $this->insert_fees_transaction_data($data,$transaction_date);
            }
            if (trim($customer_data[11]) >0 ) {
                $data = array(
                        "member_id" => $customer_data[0],
                        "member_fee_id" => 2,
                        "amount" => trim($customer_data[11]),
                        "narrative" =>"Fees Payment imported on [08-FEB-2021]",
                        "member_name" => $customer_data[2],
                        "account_id" => 20
                    );
            ...
            if ($customer_data[12] != 'NULL' && $customer_data[12] !='0') {
                $this->insert_savings_transaction_data($customer_data);
            }
            if ($customer_data[13] != 'NULL' && $customer_data[13] !='0') {
                $this->insert_share_transaction_data($customer_data);
            }
    ...
    private function insert_fees_transaction_data($transaction,$transaction_date) {  
            ...
            $trans_row = [
                "transaction_no" =>  "FEE".intval(0).date('yws').mt_rand(1000,9999),
        ...
        $transaction_data=$this->Data_import_model->add_membership_fees($trans_row);
             ...
             $single_row = [
                ...
                "ref_id" => $transaction_data['transaction_id'],
                "ref_no" => $transaction_data['transaction_no'],
                "description" =>$transaction["member_name"]." ".$transaction["narrative"] ,
                "transaction_date" => $transaction_date,
                "status_id" => 1,
                "date_created" =>time(),
                "created_by" => 1,
            ...
            $insert_id=$this->Data_import_model->add_journal_tr($single_row);
             if(!empty($insert_id)){
                ...
                $data[0] = [
                    ...
                    'reference_no'=>$transaction_data['transaction_no'],
                    'reference_id'=>  $transaction_data['transaction_id'],
                    'narrative' => $transaction["member_name"]." ".$transaction["narrative"]." made on " . $transaction_date,
