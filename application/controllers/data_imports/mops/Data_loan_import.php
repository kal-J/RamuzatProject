...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $amount= trim($loan_data[4]);
        $member_id = $loan_data[1];
        $loan_product_id =1;
        $interest_type =2;
        $repayment_frequency = trim($loan_data[9]);
        $repayment_made_every = 3;//Monthly payments
        $installments= trim($loan_data[10]);
        $interest_rate = trim($loan_data[7])*2;
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                $single_row = [
                    ...
                    "comment" => 'Loan imported from the Excel',
                ...
                if ($inserted_loan_id) {
                    ...
                    $loan_state_details = [
                        ...
                        "state_id" => $loan_state[isset($loan_data[8])?$loan_data[8]:"Active"],
                        "comment" => $loan_state_comment[isset($loan_data[8])?$loan_data[8]:"Active"],
                    ...
                    //Loan fee addition
                    if ($loan_data[3] !='NULL' && $loan_data[3] !='') {
                        $loan_fee_data = [
                                "client_loan_id" => $inserted_loan_id,
                                "loan_product_fee_id" => 1,
                                "amount" => trim($loan_data[3]),
                                "paid_or_not" => 1,
