...
class Member_import extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."nina".DIRECTORY_SEPARATOR;
    ...
    private function insert_user_data($customer_data) {
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            if ($customer_data[2]) {
                $this->do_insert_contacts($user_id,$customer_data[2]);
            }
            if ($customer_data[4]) {
                $this->insert_nextofkin($user_id,$customer_data[4]);
            }
            if ($customer_data[5]) {
                $this->insert_nextofkin($user_id,$customer_data[5]);
            }
            if ($customer_data[6]) {
                $this->insert_nextofkin($user_id,$customer_data[6]);
            }
            if ($customer_data[7]) {
            ...
            if ($customer_data[10]) {
                $this->insert_nextofkin($user_id,$customer_data[10]);
            }
            if ($customer_data[11]) {
                $this->insert_nextofkin($user_id,$customer_data[11]);
            }
