...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $date_created =date('Y-m-d',strtotime('28-02-2021'));
        $action_date = date('Y-m-d',strtotime('28-02-2021'));
        $amount=round(trim($loan_data[2]),2);
        $installments=$loan_data[3]*12;
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                ...
                if ($inserted_loan_id) {//add the client incomes
                    ...
                    //Schedule genration
                    $r=$interest_rate_per_annum=( 18/100 ); 
                    $l=$length_of_a_period=(1/$installments);
                    ...
                    for ($y=1; $y <= $installments; $y++) { 
                        ...
                        $repayment_date = date('Y-m-d',strtotime('+1 month', strtotime($repayment_date)));
                        $index_key+=2;
