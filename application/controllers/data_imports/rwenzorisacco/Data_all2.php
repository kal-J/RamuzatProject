 ...
 */
class Data_all2 extends CI_Controller {
    ...
    public function __construct() {
        ...
        // $this->member_no="NFS00000";
       
    }

    public function index() {
        //rwenzori_added_member_data.csv
        $folder = "data_extract".DIRECTORY_SEPARATOR."rwenzorisacco".DIRECTORY_SEPARATOR;
        $file_name = "added_member_not_in_shares.csv";
    ...
    private function insert_user_data($customer_data){
         ...
         if($customer_data[0] !="" && $customer_data[0] !=NULL){
             ...
             // Create non existing accounts with share/savings balance as 0 with the new number format 
            //  if($customer_data[8] !="" && $customer_data[8]!=NULL){
            //  $DateCsv = $customer_data[8];
            //  $DateCsvExploded = explode("/",$DateCsv);
            //  $transposedDate =$DateCsvExploded[2]."-".$DateCsvExploded[1]."-".$DateCsvExploded[0];
            //  $date_registered   =  $transposedDate;
            //  } 
            //  else{
                 $date_registered = '2020-12-31';
             //}
             if($customer_data[3]!="" && $customer_data[3]!=NULL){
                 if($customer_data[3]=='M'){
                     $gender=1;
                 }
                 if($customer_data[3]=='F'){
                    $gender=0;
                }
                if($customer_data[3]=='G'){
                    //Group
                    $gender=3;
                }
                
             }
           // $date_registered = $customer_data[8] !="" ? $customer_data[8]: "2020-12-31";
           
        if ($customer_data[2] != "" && $customer_data[2] != NULL) {
            $names = explode(" ", trim($customer_data[2]));
            $check_for_salutation = explode('.',$names[0]);
           // echo json_encode(count($check_for_salutation));die;
            // if(is_array($check_for_salutation)){
            //     $salutation = $check_for_salutation[0];
            //     $firstName = isset($check_for_salutation[0]) && count($check_for_salutation)>1 ? $check_for_salutation[1]:'';
            // }
            // else{
            //     $salutation='';
            ...
            $single_row = [
                ...
                "firstname" => strtoupper($names[0]),
                "lastname" => isset($names[1])?strtoupper($names[1]):'',
                "othernames" => isset($names[2])?strtoupper($names[2]):'',
                "gender" =>$gender,
                //"salutation"=>$salutation,
               // "email" => isset($customer_data[5])?strtolower($customer_data[5]):'',
               // "marital_status_id" => strtoupper($customer_data[10]=="MARRIED"?1:($customer_data[10]=="SINGLE"?2:1)),
            ...
            $member_data = [
                "user_id" => $user_id,
                "client_no" => isset($customer_data[0]) ? strtoupper($customer_data[0]):'',
                "branch_id" => 1, 
    ...
    private function insert_contacts($user_id, $customer_data) {
        if (isset($customer_data[10]) && $customer_data[10] !== "") {
            $phone_number = $customer_data[10];
    ...
    private function do_insert_contacts($user_id, $phone_number) {
     ...
     private function insert_next_of_kin($user_id,$customer_data){
        if($customer_data[11] != "" && $customer_data[11] != NULL) {
            $nextOfKinName = explode(" ",$customer_data[11]);
        }
         $data = [
            "user_id" => $user_id,
            ...
            "gender" => isset($customer_data[14]) && $customer_data[14]==1 ? "M":(isset($customer_data[14]) && $customer_data[14]==0 ? "F":""),
            "telphone"=>isset($customer_data[13]) ? $customer_data[13]:'',
            "created_by"=> 1,
            "date_created" =>time(),
            "status_id"=>1
         ];
    ...
    private function insert_user_addres($user_id,$customer_data){
         $data = [
           "user_id" => $user_id,
           "address1"=> isset($customer_data[5]) ? $customer_data[5]:'',
           "address_type_id" =>1,
           "date_created" => time(),
