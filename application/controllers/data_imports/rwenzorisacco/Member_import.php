...
class Member_import extends CI_Controller {
        ...
        if ($handle) {
            ...
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    ...
                    if ($data1[4]==2) {
                        $this->account_no2 = ++$this->account_no2;
                        $this->account_no=$this->account_no2;
                    }elseif ($data1[4]==3) {
                        $this->account_no3 = ++$this->account_no3;
                        $this->account_no=$this->account_no3;
                    }elseif($data1[4]==4){
                        $this->account_no4 = ++$this->account_no4;
                        $this->account_no=$this->account_no4;
                    }else{
                        $this->account_no1 = ++$this->account_no1;
                        $this->account_no=$this->account_no1;
    ...
    public function create_savings_accounts($member_id,$customer_data) {
        $data = [
            ...
            "status_id" =>7,
            "client_type" =>1,
            "deposit_Product_id" =>$customer_data[4],
