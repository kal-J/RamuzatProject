...
class Member_import extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->account_no="";
        $this->savings_account_no="SV000000";
        $this->account_no2="GRP000000";
        $this->account_no3="IST000000";
        $this->account_no4="JNT000000";
        $this->share_account_no="SH00000";
        $this->member_no="M000000";
    }

    public function index() {
        $this->db->trans_start();
        $folder = "data_extract".DIRECTORY_SEPARATOR."unk".DIRECTORY_SEPARATOR;
        ...
        if ($handle) {
            ...
            while (($data = fgetcsv($handle, 30048576, ",")) !== FALSE) {
                ...
                if ($count == 0) {//the row with the field_names
                    ...
                    $this->savings_account_no = ++$this->savings_account_no;
                    $this->member_no = $data1[0];
    ...
    private function insert_user_data($customer_data) {
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            $name7=isset($names[7])?$names[7]:'';
            $name8=isset($names[8])?$names[8]:'';
            $single_row = [
                "firstname" => $names[0],
                "lastname" => isset($names[1])?$names[1]:'',
                "othernames" => $name2." ".$name3." ".$name4." ".$name5." ".$name6." ".$name7." ".$name8." ",
                "gender" => $customer_data[6],
                "email" => '',
                "marital_status_id" => 1,
    ...
    public function create_savings_accounts($member_id,$customer_data) {
        $data = [
            ...
            "account_no" => $this->savings_account_no,
            "date_created" => time(),
            "created_by" => 1,
            "modified_by" => 1,
        ];
        ...
        if ($customer_data[3]<0) {
        $this->insert_transaction_data_savings_w($savings_account,$customer_data);
        ...
        $amount = abs($transaction[4]);
         //echo $transaction_date; die();
            ...
            $trans_row = [
                ...
                "narrative" => "SHARES [ " . $transaction[1]." ][ ". $transaction[0]." ]",
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1
            ];
            $transaction_data=$this->Data_import_model->add_transaction_shares($trans_row);
             ...
             $single_row = [
                ...
                "description" =>"SHARES [ " . $transaction[1]." ][ ". $transaction[0]." ]",
                "transaction_date" =>  $transaction_date,
                "status_id" => 1,
                "date_created" => time(),
                "created_by" => 1,
