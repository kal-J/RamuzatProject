...
class Data_loan_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $topUp = ["N" => 0, "Y" => 1];
        //print_r($loan_data[14]);die();
        $date_created = time();
        $action_date = $this->helpers->extract_date_time($loan_data[15],"Y-m-d");

        $amount=$loan_data[4];
        $members = $this->Data_import_model->get_member_id($loan_data[1]);
        ...
        if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
            ...
            if ($inserted_loan_id) {
                ...
                $loan_state_details = [
                    "client_loan_id" => $inserted_loan_id,
                    "state_id" => $loan_data[9]==1?10:7,
                    "comment" => $loan_data[9]==1?'Obligations met Loan closed - Data imported':'Loan Dispersed - Data imported',
...
///
                 $sent_data1['loan_fee']=[
                    ...
                    $this->do_journal_fees($sent_data1['loan_fee'],false);
