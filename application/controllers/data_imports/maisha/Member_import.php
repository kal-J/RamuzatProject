...
class Member_import extends CI_Controller {
    ...
    private function insert_user_data($customer_data) {
        $date_registered = $this->extract_date_time(isset($customer_data[10])?$customer_data[10]:"01-01-2021","Y-m-d");
        $date_of_birth = $this->extract_date_time(isset($customer_data[3])?$customer_data[3]:"01-01-2021","Y-m-d");
     
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            $names = explode(" ", trim($customer_data[2]));
            $single_row = [
                "firstname" => $customer_data[1],
                "lastname" => isset($names[0])?$names[0]:'',
                "othernames" => isset($names[1])?$names[1]:'',
                "gender" =>  isset($customer_data[7])?$customer_data[7]:'M',
                "marital_status_id" => 1,
                "children_no" => 0,
            ...
            if ($customer_data[8]) {
                $this->insert_nextofkin($user_id, $customer_data[8],$customer_data[9]);
            }
            if ($customer_data[11]) {
                $this->insert_nextofkin($user_id, $customer_data[11],$customer_data[12]);
            }
   ...
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
    ...
    private function insert_nextofkin($user_id, $names,$relationship) {
