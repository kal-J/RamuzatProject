...
class Loan_payment_import extends CI_Controller {
    ...
    private function insert_loan_data($loan_data) {
        ...
        $payment_date= $this->helpers->extract_date_time($loan_data[6],"Y-m-d");

        $payment_status = ["FULL" => 1, "PARTIAL" => 2, "PAID OFF" => 3, "PAID UP" => 3];
        // $loan_no='LN0000'+$loan_data[0];
        $schedule = $this->Data_import_model->get_reschedule_id($loan_data[0],$loan_data[1]);
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                ...
                $data = array(
                            'payment_status' => $loan_data[6],//All payments will be partial
