...
class Data_loan_import extends CI_Controller {
    ...
    public function index() {
        $folder = "data_extract".DIRECTORY_SEPARATOR."allcorps".DIRECTORY_SEPARATOR;
    ...
    private function insert_loan_data($loan_data) {
        ...
        $date_created =$this->helpers->extract_date_time($loan_data[10],"Y-m-d");
        $action_date = $this->helpers->extract_date_time($loan_data[10],"Y-m-d");
        $amount=round(trim($loan_data[2]),2);
        ...
        try {
            if ($loan_data[0] != "" && $loan_data[0] != 'NULL') {
                $single_row = [
                    ...
                    "offset_period" => 0,
                    "offset_made_every" => 1,
                    "repayment_frequency" => $loan_data[7],
                    "repayment_made_every" => $loan_data[8],
                    "installments" => $loan_data[6],
                    "link_to_deposit_account" => 1,
                    "comment" => 'Loan imported from the old system/ Excel',
                    "amount_approved" => $amount,
                    "approval_date" => $action_date,
                    "approved_installments" => $loan_data[6],
                    "approved_repayment_frequency" => $loan_data[7],
                    "approved_repayment_made_every" => $loan_data[8],
                    "approved_by" => 1,
                ...
                if ($inserted_loan_id) {//add the client incomes
                    ...
                    //Schedule genration
                        if ($loan_data[8]==1) {
                            $repayment_made_every= 30;
                            $schedule_date=' +'.$loan_data[7].' day';
                        }elseif ($loan_data[8]==2) {
                            $schedule_date=' +'.$loan_data[7].' week';
                            $repayment_made_every= 4;
                        }else{
                            $schedule_date=' +'.$loan_data[7].' month';
                            $repayment_made_every= 1;
                        }

                    $r=$interest_rate_per_annum=((($loan_data[3])*1)/100); 
                    $l=$length_of_a_period=($loan_data[7]/$repayment_made_every);
                    $i=$interest_rate_per_period=($r*$l);
                    $n=$loan_data[6]; 
                    $p=$amount;
                    $index_key=2;
                    $repayment_date=$action_date;
                    $interest_sum=0;
                    $principal_amount=0;
