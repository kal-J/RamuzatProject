...
class Member_import extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->member_no="AGD000000";
        $this->account_no="SV000000";
    ...
    private function insert_user_data($customer_data) {
        $date_registered = $this->helpers->extract_date_time($customer_data[6],"Y-m-d");
        $gender = ["M" => 1, "F" => 0];
        ...
        if ($customer_data[0] != "" && $customer_data[0] != NULL) {
            ...
            $member_data = [
                ...
                "occupation" => "",
                "registered_by" => 1,
                "status_id" => $customer_data[2]==1?1:2,
            ...
            if ($customer_data[3]) {
                $this->insert_address($user_id,$customer_data[3]);
            }
            $status_id=$customer_data[2]==1?7:17;
           $this->create_savings_accounts($member_id,$status_id);
            return 1;
        }
        return 0;
    }
    ...
    public function create_savings_accounts($member_id,$status_id) {
        $data = [
            "member_id" => $member_id,
            "status_id" =>7,
            "client_type" =>1,
        ...
        $savings_account=$this->Data_import_model->add_savings_account($data,$status_id);
        
        $this->create_share_accounts($member_id,$savings_account,$status_id);
    }
    
    private function create_share_accounts($member_id,$savings_account,$status_id) {
        ...
        $this->load->model("Data_import_model");
        $this->Data_import_model->set_share_state($data,$status_id);
    }
