<?php
     ...
     *  */
    class Journal_transaction_model extends CI_Model {
        ...
        public function __construct() {
            /**
             * To be able to delete a transaction entry of a given journal type. the following query can be run
             * delete from fms_journal_transaction_line where journal_transaction_id in (select id from fms_journal_transaction where journal_type_id=16 AND ref_id in (2,3,4));
             */
        ...
        public function get2($filter = FALSE) {
            $tt_amount_subquery = "(SELECT `journal_transaction_id` `jt_id`, SUM(`debit_amount`) `tt_amount`, SUM(`credit_amount`) `c_amount` FROM `fms_journal_transaction_line` WHERE status_id=1  GROUP BY `journal_transaction_id`) tt_as";
        ...
        public function get_dTable() {
            ...
            if (!empty($_POST['start_date'])) {
                ...
                $this->db->where('jtr.transaction_date >=' . $start_date);
            }
    
            if (!empty($_POST['end_date'])) {
                $end_date = str_replace('-', '', $_POST['end_date']);
                $this->db->where('jtr.transaction_date <= ' . $end_date);
            }
      ...
      public function get_found_rows() {
        ...
        public function reverse($ref_id=false,$ref_no=false,$journal_type_id=false,$id=false) {
            ...
            if(!$this->should_reverse($id=false,$ref_id, $ref_no)){
                return false;
            }
        ...
        public function should_reverse($id=false,$ref_id=false,$ref_no=false){
        ...
        public function reverse_main($data,$id) {
         ...
         public function reverse_lines($data,$id) {
            $this->db->where('journal_transaction_id='.$id);
