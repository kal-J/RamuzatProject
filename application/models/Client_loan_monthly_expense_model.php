<?php
class Client_loan_monthly_expense_model extends CI_Model {

public function __construct() {
  $this->load->database();
}

var $table = 'client_loan_monthly_expense';

  //save picture data to db
  function set($user_data = []){
    if ($user_data === []) {
    $user_data['client_loan_id'] = $this->input->post('client_loan_id');
    $user_data['expense_id'] = $this->input->post('expense_id');
  ...
  public function set2($loan_id=false) {
        ...
        foreach ($expenses as $key => $value) {//it is a new entry, so we insert afresh
...
public function get( $filter = FALSE ) {
  $this->db->select( ' a.id,b.expense_type, a.expense_id, a.amount, a.description, a.status_id' );
  $this->db->from( $this->table.' a' );  
  $this->db->join('user_expense_type b', 'a.expense_id = b.id', 'LEFT');
